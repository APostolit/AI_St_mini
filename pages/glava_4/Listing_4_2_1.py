...
def my_car(car):
    if car == 1:
        print('Автомобиль может ехать ----------->')
    else:
        print('Автомобиль должен стоять <-----------')

# Исходные данные для операции AND
X = np.array([[0, 0], [0, 1], [1, 0], [1, 1]])
# Правильные ответы для операции AND
y = np.array([0, 0, 0, 1])
...
print('Свет красный, есть пешеходы x=[0, 0]')
x = [0, 0]
my_car(model.predict(x))

print('Свет зеленый, есть пешеходы x=[1, 0]')
x = [1, 0]
my_car(model.predict(x))

print('Свет красный, нет пешеходов x=[0, 1]')
x = [0, 1]
my_car(model.predict(x))

print('Свет зеленый, нет пешеходов x=[1, 1]')
x = [1, 1]
my_car(model.predict(x))
