...
st.text("На 5 (очень хорошая погода) - " + str(prediction))

X_test = np.array([[5, 4, 50, 2]])
prediction = model.predict(X_test)
st.text("На 4 (хорошая погода) - " + str(prediction))

X_test = np.array([[10, 7, 45, 3]])
prediction = model.predict(X_test)
st.text("На 3 (средняя погода) - " + str(prediction))

X_test = np.array([[15, 9, 21, 4]])
prediction = model.predict(X_test)
st.text("На 2 (плохая погода) - " + str(prediction))

X_test = np.array([[20, 11, 10, 6]])
prediction = model.predict(X_test)
st.text("На 1 (очень плохая погода) - " + str(prediction))